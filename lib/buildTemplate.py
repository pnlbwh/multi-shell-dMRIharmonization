...
def applyXform(inImg, refImg, warp, trans, outImg):
    (
        antsApplyTransforms[
            "-d", "3", "-i", inImg, "-o", outImg, "-r", refImg, "-t", warp, "-t", trans
        ]
        & FG
    )
...
def warp_bands(imgPath, maskPath, templatePath):
    ...
    for i in range(0, N_shm + 1, 2):
        applyXform(
            pjoin(directory, "harm", f"{prefix}_L{i}.nii.gz"),
            pjoin(templatePath, "template0.nii.gz"),
            warp,
            trans,
    ...
    for dm in diffusionMeasures:
        applyXform(
            pjoin(directory, "dti", f"{prefix}_{dm}.nii.gz"),
            pjoin(templatePath, "template0.nii.gz"),
            warp,
            trans,
...
def antsMult(caselist, outPrefix):
    ...
    N_core = 16  # getenv('TEMPLATE_CONSTRUCT_CORES')
    check_call(
        (" ").join(
            [
                pjoin(
...
def dti_stat(siteName, imgs, masks, templatePath, templateHdr):
    ...
    if not isfile(morphed_mask_name):
        ...
        morphed_mask = (
            binary_opening(
                np.mean(maskData, axis=0) > 0.5,
                structure=generate_binary_structure(3, 1),
            )
            * 1
        )
        save_nifti(
            morphed_mask_name, morphed_mask.astype("uint8"), templateAffine, templateHdr
        )
    ...
    for dm in diffusionMeasures:
        ...
        for imgPath in imgs:
            prefix = basename(imgPath).split(".nii")[0]
            imgData.append(
                load_nifti(pjoin(templatePath, f"{prefix}_Warped{dm}.nii.gz"))[0]
            )

        save_nifti(
            pjoin(templatePath, f"Mean_{siteName}_{dm}_b{bshell_b}.nii.gz"),
            np.mean(imgData, axis=0),
            templateAffine,
            templateHdr,
        )

        save_nifti(
            pjoin(templatePath, f"Std_{siteName}_{dm}_b{bshell_b}.nii.gz"),
            np.std(imgData, axis=0),
            templateAffine,
            templateHdr,
        )
...
def rish_stat(siteName, imgs, templatePath, templateHdr):
    for i in range(0, N_shm + 1, 2):
        ...
        for imgPath in imgs:
            prefix = basename(imgPath).split(".nii")[0]
            imgData.append(
                load_nifti(pjoin(templatePath, f"{prefix}_WarpedL{i}.nii.gz"))[0]
            )

        templateAffine = templateHdr.get_best_affine()
        save_nifti(
            pjoin(templatePath, f"Mean_{siteName}_L{i}_b{bshell_b}.nii.gz"),
            np.mean(imgData, axis=0),
            templateAffine,
            templateHdr,
        )
        ...
        save_nifti(
            pjoin(templatePath, f"Std_{siteName}_L{i}_b{bshell_b}.nii.gz"),
            np.std(imgData, axis=0),
            templateAffine,
            templateHdr,
        )
...
def template_masking(refMaskPath, targetMaskPath, templatePath, siteName):
    ...
    if not isfile(pjoin(templatePath, "templateMask.nii.gz")):
        save_nifti(
            pjoin(templatePath, "templateMask.nii.gz"),
    ...
    for dm in diffusionMeasures:
        ...
        img = load(fileName)
        save_nifti(
            fileName, applymask(img.get_data(), templateMask), img.affine, img.header
        )
...
def stat_calc(ref, target, mask):
    ...
    np.nan_to_num(per_diff).clip(max=100.0, min=-100.0, out=per_diff)
...
def difference_calc(
    refSite, targetSite, refImgs, targetImgs, templatePath, templateHdr, mask, measures
):
    """
    if traveling heads:
        for each subject:
    ...
    for dm in measures:
        ...
        if travelHeads:
            ...
            for refImg, targetImg in zip(refImgs, targetImgs):
                ...
                target = load_nifti(pjoin(templatePath, f"{prefix}_Warped{dm}.nii.gz"))[
                    0
                ]
        ...
        save_nifti(
            pjoin(templatePath, f"Delta_{dm}_b{bshell_b}.nii.gz"),
        ...
        save_nifti(
            pjoin(templatePath, f"PercentageDiff_{dm}_b{bshell_b}.nii.gz"),
            np.mean(per_diff, axis=0),
            templateAffine,
            templateHdr,
        )

        save_nifti(
            pjoin(templatePath, f"PercentageDiff_{dm}smooth_b{bshell_b}.nii.gz"),
            np.mean(per_diff_smooth, axis=0),
            templateAffine,
            templateHdr,
        )
        ...
        if "L" in dm:
            save_nifti(
                pjoin(templatePath, f"Scale_{dm}_b{bshell_b}.nii.gz"),
                np.sqrt(np.mean(scale, axis=0)),
                templateAffine,
                templateHdr,
            )
