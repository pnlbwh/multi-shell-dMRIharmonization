...
def resize_spm(lowResImg, inPrefix):
    ...
    p = Popen(
        (" ").join([pjoin(SCRIPTDIR, "spm_bspline_exec", "bspline"), inPrefix]),
        shell=True,
    )
    p.wait()
...
def save_high_res(fileName, sp_high, lowResImgHdr, highResImg):
    ...
    scale = np.diag((sp_high / sp_low).tolist() + [1.0])
...
def resampling(
    ...
    if interp_toolbox == "scipy":
        ...
        # resample the mask
        highResMask = resize(
            lowResMask.astype("float"), (sx, sy, sz), order=1, mode="symmetric"
        )  # order 1 for linear interpolation

    elif interp_toolbox == "spm":
        step = sp_high / sp_low
        sx, sy, sz = [
            int(x) for x in ((lowResImg.shape[:3] + step + 0.01 - 1) / step + 1)
        ]
    ...
    highResMask = (
        binary_opening(highResMask >= 0.5, structure=generate_binary_structure(3, 1))
        * 1
    )
    ...
    np.nan_to_num(b0HighRes).clip(
        min=0.0, out=b0HighRes
    ...
    np.nan_to_num(b0_gibs).clip(min=0.0, out=b0_gibs)  # using min= 1. is unnecessary
