...
def find_b0(dwi, where_b0, mask=None):
    ...
    np.nan_to_num(b0).clip(min=0.0, out=b0)
...
def normalize_data(dwi, where_b0=None, mask=None, b0=None):
    ...
    if where_b0 is not None and b0 is None:
        ...
        np.nan_to_num(b0).clip(min=1.0, out=b0)  # can be changed to 0. as well
        for i in where_b0:
            dwi[..., i] = b0
    else:
        np.nan_to_num(b0).clip(min=1.0, out=b0)  # can be changed to 0. as well
    ...
    np.nan_to_num(dwiPrime).clip(min=0.0, max=1.0, out=dwiPrime)
