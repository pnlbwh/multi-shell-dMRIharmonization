...
def register_subject(imgPath, warp2mni, trans2mni, templatePath, siteName, bshell_b):
    ...
    outPrefix = pjoin(
        templatePath, basePrefix.replace(f"_b{bshell_b}", "_FA_ToTemplate")
    )  # should have _FA at the end
...
def main():
    ...
    outPrefix = pjoin(
        templatePath, header
    )  # siteName+ f'_b{bshell_b}'+ header.split(siteName)[-1])
