...
def separateBshells(imgPath, ref_bvals_file=None, ref_bvals=None):
    ...
    for bval in ref_bvals:
        ...
        if bval == 0.0:
            ...
            b0_bshell = np.zeros(
                (dwi.shape[0], dwi.shape[1], dwi.shape[2], N_b + 1), dtype="float32"
            )
            b0_bshell[:, :, :, 0] = b0
            b0_bshell[:, :, :, 1:] = dwi[:, :, :, ind]

            b0_bvals = [0.0] + [bval] * N_b
            ...
            print(
                "bvals difference", (np.array(b0_bvals) - np.array(bvals_written)).sum()
            )
            print(
                "bvecs difference", (np.array(b0_bvecs) - np.array(bvecs_written)).sum()
            )
