...
class multi_shell_pipeline(cli.Application):
    ...
    def main(self):
        ...
        ## separate b-shells
        if self.ref_csv:
            refListOutPrefix = separateShellsWrapper(
                self.ref_csv, ref_bvals_file, self.N_proc
            )
        if self.target_csv:
            tarListOutPrefix = separateShellsWrapper(
                self.target_csv, ref_bvals_file, self.N_proc
            )
                ...
                check_call(
                    (" ").join(
                        [
                            ...
                            str(int(bval)),
                            "--ref_list",
                            refListOutPrefix + f"_b{int(bval)}.csv",
                            "--create",
                        ]
                        + pipeline_vars
                    ),
        ...
        if self.process:
            ...
            if self.debug and self.ref_csv:
                joinAllBshells(
                    self.ref_csv, ref_bvals_file, "reconstructed_", self.N_proc
                )
