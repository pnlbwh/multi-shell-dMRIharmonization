...
def main(config_file):
    ...
    try:
        ...
        logging.error(f'An error occurred while running the download_from_s3 script: {e}')
        sys.exit(1)

    # Run the write_local_paths script
    try:
        subprocess.run(f"python write_local_paths.py -d {reference_dir} -o {config['local_paths']['reference_output']}", shell=True,
                       check=True)
        subprocess.run(f"python write_local_paths.py -d {target_dir} -o {config['local_paths']['target_output']}", shell=True, check=True)
        logging.info('Successfully ran the write_local_paths script.')
    except subprocess.CalledProcessError as e:
        logging.error(f'An error occurred while running the write_local_paths script: {e}')
        sys.exit(1)
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Automated MRI data harmonization pipeline.')
